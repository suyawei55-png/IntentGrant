...
- **Deliverability → Delivery Receipt / Transit Fidelity Receipt (§4.7)**: the weak-surface Delivery Receipt (D1, the procedural semantics of a disclosure being presented conspicuously in the manner required by this specification — proving that the presentation event and an opportunity for review occurred, without promising that the user actually read or understood it) and the Transit Fidelity Receipt (D2, a three-segment chain proving that the transit party did not swap the disclosed content), covering the gap of "stored version ≠ displayed version".
- **Composition semantics**: the complete evidence chain in one sentence — **the Disclosure Object contained every element required by the applicable baseline; the same disclosure remained intact throughout the chain; that same disclosure was actually presented conspicuously in the manner required by this specification for the user's review; and the person affirmatively authorized the specific action as presented (with evidentiary strength governed by LoA)** (§6.1, the four-element model and object continuity rule).
...
| --- | --- | --- | --- |
| ① Sufficiency | Was the Disclosure Object complete with respect to all elements required by the applicable baseline? | **The Schema Registry in this section** | New in v0.2 |
| ② Integrity | Did the same disclosure remain unaltered throughout the chain? | disclosure_hash + signature + custody anchoring | Core of v0.1.1 |
| ③ Deliverability | Was that same disclosure actually presented conspicuously in the manner required by this specification for the user's review? | D0 rendering trace (v0.1.1, audit grade) → D1/D2 delivery receipts (v0.2 §4.7, device-signature grade) | Upgraded in v0.2 |
| ④ Authenticity of intent | Did the person affirmatively authorize the specific action as presented? | IntentGrant (physical confirmation) | Core of v0.1.1 |

The complete evidence chain in one sentence: **the Disclosure Object contained every element required by the applicable baseline; the same disclosure remained intact throughout the chain; that same disclosure was actually presented conspicuously in the manner required by this specification for the user's review; and the person affirmatively authorized the specific action as presented (with evidentiary strength governed by LoA).**

**Object continuity (identity binding, made explicit in v0.2.1)**: the four elements **MUST** bind to the same Disclosure Object — the same `disclosure_hash` flows from construction in §3.1 through challenge binding (§3.3), the Delivery Receipt (§4.7), and Receipt alignment (§4.5). The verifier **MUST** check that the evidence for all four elements references the same `disclosure_hash`: if complete version A is placed in custody, truncated version B is presented, and an action with different parameters C is confirmed, the resulting object drift is a mismatch under this rule (§4.5, VIOLATED). This makes an existing mechanism property explicit in v0.2.1; it is not a new mechanism.

**Honest boundary**: deliverability proves that the disclosure event occurred in the manner required by this specification and that an opportunity for review was established; it does not prove that the user actually read or understood the disclosure. “Conspicuous” describes the strength of the presentation obligation (§9.2 statutory mapping), not a promise about reading outcomes.

Key point: the four elements are mutually independent, and if one link is missing the chain is blind on the corresponding question; the four elements nevertheless bind to the same Disclosure Object (object continuity), and any object drift is a mismatch. What the schema supplies is link ① — it does not compete with the existing mechanisms; rather, it gives `disclosure_hash` a "content floor": v0.1.1's `disclosure_hash` solves **integrity** (the disclosure remained intact), not **sufficiency** (the disclosure was complete) — anchoring a hash to an insufficient disclosure is like stamping a seal across the pages of a non-compliant disclosure.
...
| v0.2.1 | 2026-09-25 | **Cross-reference errata; no mechanism-semantic change.** Corrected six statements that attributed a CA-certified legal-effect path to Core v2.0; Core's current capability is endpoint private-key signing, while a CA-certified financial-grade route remains a future Profile. Disambiguated Core architecture Layer 0–Layer 3 from LoA L0–L3, including audit-layer terminology and pseudocode helper names. Made the PAE, Controller App, and Control Plane definitions self-contained instead of relying on a non-normative upstream glossary. **⑤ Made object continuity across the four elements explicit and narrowed the deliverability claim**: all four elements bind to the same `disclosure_hash` (object-continuity rule added to §6.1); deliverability is stated as conspicuous presentation in the manner required by this specification, rather than as a "prominent viewing opportunity". The Abstract, the §6.1 table and final sentence, and the §11.1 terminology are updated accordingly. The Chinese text remains normative, and the English edition now carries the corresponding translation. |
